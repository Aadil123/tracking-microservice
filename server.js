...
app.get("/tracking/details/:orderId", async (req, res) => {
  try {
    ...
    if (!trackingDetails) {
      return res
        .status(404)
        .send("Tracking details not found for the given orderId");
    }
    ...
    ); // Update with actual Order Microservice URL
    const orderData = orderResponse.data;

    // Step 3: Fetch user details from User Microservice
    const userResponse = await axios.get(
      `http://localhost:5000/user/${orderData.userId}`,
      { headers: getRequestHeaders(req) }
  ...
  try {
    ...
    if (!status && !location) {
      return res
        .status(400)
        .send(
          "At least one field (status or location) must be provided to update."
        );
    }
    ...
    if (!updatedTracking) {
      return res
        .status(404)
        .send("Tracking data not found for the given orderId.");
    }
...
app.get("/tracking/status/:status", async (req, res) => {
  try {
    ...
    if (!records.length) {
      return res
        .status(404)
        .send("No tracking data found for the given status");
    }
...
app.get("/tracking/location/:location", async (req, res) => {
  try {
    ...
    if (!records.length) {
      return res
        .status(404)
        .send("No tracking data found for the given location");
    }
